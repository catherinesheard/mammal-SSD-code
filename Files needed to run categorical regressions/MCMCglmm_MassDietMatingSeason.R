MCMCglmm_MassDietMatingSeason_Separate<-function(phylo,data,savefile){
  ...
  model.structure<-MCMCglmm(zSSD~trait:zMass+trait:zPlants+trait:zVert+
                              trait:as.factor(MatingSystem.updated)+
                              trait:zPrecipVar+trait:zTempVar+trait, 
  ...
  for(i in 1:length(phylo)){
    singletree<-phylo[[i]]  
    singletree<-nnls.tree(cophenetic(singletree),singletree,rooted=TRUE) #the tree is slightly non-ultrametric, which is annoying, so we force it to be ultrametric
